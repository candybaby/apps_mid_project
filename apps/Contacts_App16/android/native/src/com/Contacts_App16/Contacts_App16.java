...
import com.Contacts_App16.MQService.MQBinder;
import com.worklight.androidgap.WLDroidGap;

import edu.ntut.csie.mqtt.plugin.LogUtil;

public class Contacts_App16 extends WLDroidGap {
...
/*			task.post(new Runnable() {
			...
			task.post(new Runnable() {
				...
				@Override
				public void run() {
					Contacts_App16.this.bindService(new Intent(Contacts_App16.this,
		...
		task.post(new Runnable() {
			...
			public void run() {
				...
				try {
					SharedPreferences sp = Contacts_App16.this
					...
					if (StringUtils.isNotBlank(message)) {
						Contacts_App16.this.callMessageHandlerRouters(message);
					}
					SharedPreferences.Editor spEditor = sp.edit();
	...
	public void onPause() {
		...
		if (mqsConnected) {
			task.post(new Runnable() {
				...
				@Override
				public void run() {
					Contacts_App16.this.unbindService(conn);
				}
			});
	...
	private void loadResource() {
		...
		// @Override
		// public void run() {
		MQTTResources.init(Contacts_App16.this);
		// }
		// });
	}
	...
	private class MQServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			Contacts_App16.this.mqs = ((MQBinder) service).getService();
			if (Contacts_App16.this.clientId != null
					&& Contacts_App16.this.topic != null) {
				Contacts_App16.this.task.post(Contacts_App16.this.connectMQ);
			}
			Contacts_App16.this.mqsConnected = true;
		}

		@Override
		public void onServiceDisconnected(ComponentName name) {
			Contacts_App16.this.mqsConnected = false;
		}
	...
	private class MQConnectTask implements Runnable {
		...
		@Override
		public void run() {
			SharedPreferences sp = Contacts_App16.this.getSharedPreferences(
					"PersistenceSharedPreferences", Context.MODE_PRIVATE);
			SharedPreferences.Editor spEditor = sp.edit();
			spEditor.putString("clientId", clientId).commit();
			spEditor.putString("topic", topic).commit();
			if (Contacts_App16.this.mqs.connect(clientId, topic)) {
				task.post(new Runnable() {
					@Override
					public void run() {
						NotificationManager mNotifMan = MQTTResources
	...
	public void publishMessage(final String topic, final String message,
		...
		task.post(new Runnable() {
			...
			@Override
			public void run() {
				Contacts_App16.this.mqs.publishMessage(topic, message, qos);
			}
		});
	}
}
